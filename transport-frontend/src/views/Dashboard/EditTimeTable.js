...
export default function EditTimeTableView(props) {
    ...
    const[busId, setBusId] = useState(props.row.busId.busNumber);
    const[routeId, setRouteId] = useState(props.row.routeId.routeName);
    const[destination, setDestination] = useState(props.row.destination);
            ...
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => {
                        API.post(`/timeTable/delete`,{id:props.row._id})
                            .then(()=>{
                            });
                    }
                },
    ...
    const onSubmit = () => {
        ...
        console.log(data)
        API.post("/timeTable/update", data)
            .then((res) => {
                console.log(res.data)
                confirmAlert({
                    title: 'Updated Successfully',
                    message: 'Time Table Data has updated successfully',
                    buttons: [
                        {
                            label: 'Ok',
                            onClick: () => {
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={6}>
                            <Autocomplete
                                ...
                                defaultValue={busNames.find(v => v.label==busId )}
